...
calcular_ste_id <- function(id, tamano_ventana, solapamiento) {
  signal <-unlist(df[which(id==df_train[,1]),])
  longitud_signal <- length(signal)
  energia_ste <- numeric(0)  # Vector para almacenar la energía STE
...
Ahora se computan las metricas relacionadas con la amplitud de la señal

```{r}

data_NA_train <- df_NA[train,]
media <- c()
des <- c()
per25 <- c()
per75 <-c()
...
df_t %>% ggplot(aes(group=clase,y=std))+
  geom_boxplot()
...
```{r}
gabor <- function(id,w){
  elem <- unlist(df[which(id==df_train[,1]),])
...
df_train %>%ggplot(aes(group=clase,y=(gabor_75)))+
  geom_boxplot()
...
xgb_model <- xgboost(data = dtrain, params = params, nrounds = 15)
